...
void man(char * name)
{
	...
	else if(strcmp(name,"read")==0)
	{
		...
		printf("Usage : read File_name_No_of_Bytes_To_Read\n");
	}
	...
	else if(strcmp(name,"clodeall")==0)
	{
...
void DisplayHelp()
{
	printf("ls : To list aut all files\n");
	...
	printf("fstat ; To Display information od filr using file descriptor \n");
...
int main()
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			{
				printf("Terminate The Customized Virtual File System\n");
				break;
			}
			else
			{
		...
		else if(count == 2)
		{
			if(strcmp(command[0],"stat") == 0)
			{
				...
				if(ret == -1)
					printf("ERROR : Incorrect Parameaters\n");
				if(ret == -2)
					printf("ERROR : There id no such file\n");
				continue;
			}
			...
			else if(strcmp(command[0],"truncate") == 0)
			{
				...
				if(ret == -1)
					printf("Error : Incorrect parameater\n");
			}
			else
			{
				printf("\nERROR : Command not found!!!\n");
